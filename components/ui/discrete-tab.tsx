...
export function DiscreteTabs({ tabs, activeTab, onChange }: DiscreteTabsProps) {
  return (
    <div className="flex gap-4 items-center justify-center p-1.5 rounded-full mb-8">
      {tabs.map((tab) => (
        <Button
          key={tab.id}
          id={tab.id}
...
function Button({
  ...
  const activeColor = "text-zinc-900 dark:text-zinc-100";
  ...
  return (
    ...
    >
      <motion.div
        ...
        className={cn(
          "flex items-center font-mono uppercase gap-1.5 bg-zinc-100 dark:bg-zinc-800 text-zinc-500 overflow-hidden transition-colors duration-75 ease-out p-3 cursor-pointer outline-none border border-transparent shadow-none hover:shadow-none outline-transparent hover:outline-transparent",
          isActive && activeColor,
          isActive ? "px-4 bg-white dark:bg-zinc-700 shadow-sm border border-zinc-200 dark:border-zinc-700/50" : "px-3"
        )}
        style={{
          borderRadius: "25px",
        }}
        ...
        {isActive && (
          ...
          >
            <motion.span
              ...
              className="text-sm font-bold font-mono uppercase whitespace-nowrap relative inline-block text-zinc-900 dark:text-zinc-100"
              style={{ willChange: "transform" }}
            >
              {title}
            </motion.span>
